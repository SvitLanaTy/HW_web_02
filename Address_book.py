...
class AddressBook(UserList):
    ...
    def serialize_to_json(self, record: Record):
        serialize_record = {'Name': record.name.get_name,
                            'ID': record.id.get_id,
                            'Phones': [item.get_phone for item in record.phones if item],
                            'Birthday': record.birthday.get_birthday.strftime('%Y-%m-%d') if isinstance(
                                record.birthday.get_birthday, date) else None,
                            'Email': record.email.get_email,
                            'Comment': record.comment.get_comment,
                            'Address': record.address.get_address,
                            'Company': record.company.get_company,
                            'Tags': [item.get_tag for item in record.tags if item],
                            }
    ...
    def deserialize(self, dict) -> Record:
        ...
        [record.phones.append(Phone(item)) for item in dict['Phones'] if item]
        ...
        if 'Tags' in dict:
            record.tags.extend([Tag(item) for item in dict['Tags'] if item])
    ...
    def add_teg(self, record: Record, serialize_record, tag):
        ...
        self.save_contacts()
        return f"{positive_action('Tag:')} {book_style(tag)} {positive_action('added')}"
    ...
    def find_record(self, name) -> Record:
        records = []
        normalized_name = name.capitalize()  # Normalize input name

        for item in self.data:
            if item.name.get_name == normalized_name:  # Assuming `get_name` gets the name attribute
                records.append(item)

        if len(records) > 1:
            return records
            # self.show_records(records)
            # contact_id = int(input(command_message('Enter Contact ID: ')))
            # return self.find_record_id(contact_id, normalized_name)  # Pass the name for context
        elif len(records) == 0:
            raise ValueError(f'No records found for {name}!')
        else:
            return records[0]    
