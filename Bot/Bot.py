...
class MyCmd(cmd.Cmd, BotCommand):
    book = AddressBook()
    ...
    def note_table(self, notes = None, *args):
        rows = self.notes_book if not notes else notes
        table = Table()
    ...
    def do_make_note(self, *args):
        ...
        if note and 3 <= len(title) <= 10:
            ...
            print(error_message("The title must have from 3 to 10 characters and note must be entered"))
    ...
    def do_find_note(self, *args):
        ...
        if question != "":
            res = self.notes_book.find_note(question)
            if len(res) > 0:
                table = self.note_table(res) 
                self.console.print(table)            
            else:
                print(error_message("No found notes."))

    def do_find_tag(self, *args):
        "Search by tags"
        question = input(command_message("Enter your request>>> "))
        if question != "":
            res = self.notes_book.find_tag(question)
            if len(res) > 0:
                table = self.note_table(res) 
                self.console.print(table)
            else:
                print(error_message("Not found notes."))
    ...
    def do_edit_note(self, *args):
        ...
        if len(self.notes_book.find_note_id(inp_id)) >= 1:
            ...
            if note != "":
                ...
                self.notes_book.save_note()
                print(positive_action("Note was edited."))
            else:
                print(error_message("Note is empty."))
        else:
            print(error_message("Not found ID."))
    ...
    def do_edit_title(self, *args):
        ...
        if len(self.notes_book.find_note_id(inp_id)) >= 1:
            ...
            if title != "" and 3 < len(title) < 10:
                ...
                self.notes_book.save_note()
                print(positive_action("Title was edited."))
            else:
    ...
    def record_table(self, records = None, *args):
        rows = self.book if not records else records
        table = Table()
        table.add_column("ID", style="bright_magenta")
    ...
    def do_find_rec(self, *args):
        ...
        question = input(command_message("Enter your request>>> "))
        if question != "":
            res = self.book.find(question)
            if len(res) > 0:
                table = self.record_table(res)
                self.console.print(table)                        
            else:
                print(error_message("Not found record."))

    def do_del_rec(self, *args):
        "Deletes record in addressbook"
        question = input(command_message("Enter name to delete>>> "))
        ...
        if isinstance(record_to_edit, Record):
            ...
            if edit_old_ph and edit_new_ph:
                if exit_record:                    
                    ...
                    try:
                        self.book.edit_phone(record_to_edit, exiting_record_str, edit_old_ph, edit_new_ph)
                        print(positive_action("Phone edit successful ops"))                        
    ...
    def do_edit_ph_rec(self,*args):
        ...
        if question_name != "":
            ...
            if isinstance(record_to_edit, Record):
                ...
                table = self.record_table(record_to_edit)
                self.console.print(table)
                ...
                if exit_record is not None:
                    ...
                    try:
                        if self.book.edit_teg(record_to_edit, exiting_record_str, edit_old_ph, edit_new_ph):
                            print(positive_action("Tag edit successful aa"))
                        else:
                            print(error_message("Tag not found aa"))
                    except Exception as ve:
                    ...
                    try:
                        ...
                        if self.book.edit_teg(
                                record_to_edit, exiting_record_str, edit_old_ph, edit_new_ph):
                            print(positive_action("Tag edit successful bb"))
                        else:
                            print(error_message("Tag not found bb"))
                    except Exception as ve:
                        print(error_message("No changes made to the tag nabaa"))
            else:
                print(error_message("No changes made to the tag aaab"))

    # зміна тегу (працює разом з <<<def edit_tag>>>)
    def do_edit_tag_rec(self, *args):
        "Edites phone to record in addressbook by name"
        question_name = input(command_message(
            "Enter the name to edit tag>>> "))
        if question_name != "":
            record_to_edit = self.book.find_record(question_name)
            if isinstance(record_to_edit, Record):
    ...
    def do_cong_rec(self, *args):
        ...
        if question != "":
            res = self.book.congratulation(question)
            if len(res) > 0:
                table = self.record_table(res)
                self.console.print(table)
            else:
                print(error_message(f"Not found record with {question} days to birthday."))
    ...
    def do_add_tag_rec(self, *args):
        ...
        if question_name != "":
            ...
            if isinstance(record_to_edit, Record):
                ...
                print(error_message(
                    f"Not found record with the name: {question_name}"))
    ...
    def do_remove_tag_rec(self, *args):
        ...
        if question_name != "":
            ...
            if isinstance(record_to_edit, Record):
                ...
                print(error_message(
                    f"Not found record with name: {question_name}"))
